...
-------------------------------------------------------------------------------------------------------------------
    - spec/rails_helper.rb
     1. Add requirements of Capybara's poltergeist
     - require 'capybara/poltergeist'

     2. Add Capybara's configurations
     ...
     -      inspector: true,
     -      js_errors: true,
     -      widnow_size: [1024, 768]
     -   )
     - end
     -
     - Capybara.javascript_driver = :poltergeist

     3. Add Capybara to RSpec
     ...
     4. Change transactionals fixtures to false
     - RSpec.configure do |config|
     -   ...
     ...
     5. Chek that everything still works correctly for testing
     $ rspec
     # output: count of examples and failures, as before


    - spec/features/restaurants_spec.rb
     1. Create this file with requirement of rails helper
     ...
     3. Add case for checking of adding restaurants title to list on root page
     ...
     4. Run rspec and ensure that test passed
     $ rspec
...
-------------------------------------------------------------------------------------------------------------------
    - spec/rails_helper.rb
     #. Add configurations of DatabaseCleaner inside RSpec
     - RSpec.configure do |config|
     -   ...
    ...
    - spec/models/restaurant_spec.rb
     1. Add test for to_s method inside Restaurant
     ...
     2. Run rspec
     $ rspec
     # output: 100% coverage and finished example

    - spec/models/comment_spec.rb
     1. Create this file with requirement of rails helper
     - require 'rails_helper'

     2. Describe Comment model
     - describe Comment do
     - end

     3. Add case for ancestry checking
     - it 'has parent/child relationship' do
     -   restaurant = Restaurant.create(name: 'My Restaurant')
     -   parent_comment = Comment.create!(body: 'parent', restaurant: restaurant)
     -   child_comment = Comment.create!(body: 'child', parent: parent_comment, restaurant: restaurant)
     -   expect(parent_comment.children.first).to eq(child_comment)
     - end

     4. Run rspec
     $ rspec
     # output finished example witout failures

     5. change structure of restaurant variable and add its above case
     ...
     6. Run rspec to check that everything still works as before
     $ rspec
     # output: no failures, as before

     7. Check not covered place in comment.rb
     $ COVERAGE=1 rspec
     # browser: find comment.rb model and check place with red lines

     8. Add variables for parent comment and child comment, child comment should has (!) after 'let' word
     ...
     9. Clean up records creating from cases and instead just use declared variables
     - it 'has parent/child relationship' do
     -    expect(parent_comment.children.first).to eq(child_comment)
     - end

     10. Add case for upvotes checking
     - it 'upvotes' do
     -    Comment.upvote(child_comment.id)
     -    expect(child_comment.reload.rank).to eq(1)
     -  end

     11. Check that test works fine
     $ rspec
     # output: 100% coverage  and finished example

     12. Add case for checking of rank's default value
     - it 'has a default value of 0 for tank when created' do
     -   expect(parent_comment.rank).to eq(0)
     - end

     13. Check test in console
     $ rspec
     # output: failed test

    - app/models/comment.rb
     1. Add defaults function to make rank equal to 0
     - def defaults
     -   self.rank ||= 0
     - end

     2. Add before action for defaults function
     - before_save :defaults

     3. Check this test in console
     $ rspec
     # output: passed test
-------------------------------------------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------------------------------------------
    - Gemfile
     #. Ensure that you have simplecov gem in your Gemfile
    ...
    - spec/rails_helper.rb
     #. Add requirement and start configuration of simplecov gem
     - require 'simplecov'
     - SimpleCov.start

    - spec/sanity_spec.rb
     1. Add requirement of reails helper
     - require 'rails_helper'

     2. run spec in console
     $ rspec
     # output: procent of coverage (for now 100%)

    - spec/models/restaurants_spec.rb
     1. Create file and add requirement of rails helper
     - require 'rails_helper'

     2. Describe Restaurant model
     - describe Restaurant do
     - end

     3. Check procent of coverage
     $ rspec
     # output: new count of LOCs and new procent of coverage

    - coverage/index.html
     #. Open this file to look by it

    - spec/rails_helper.rb
     1. Add eager_load for coverage
     - Rails.application.eager_load! if ENV['COVERAGE']

     2.check 1 coverage
     $ COVERAGE=1 rspec
     # output: coverage procent of every model and controller

-------------------------------------------------------------------------------------------------------------------
                    Setting Up Rspec
-------------------------------------------------------------------------------------------------------------------
    - Gemfile
     1. Add the gems to development test group
     # - gem 'capybara'
     # - gem 'rspec'
     # - gem 'rspec-rails', '~> 3.5'
    ...
    - /
     ...
     2. Generate rspec gem's stuff
     $ rails g rspce:install
     # output: generated spec folder with files

     3. Check that rspec is works
     $ rspec
     # output: count of examples and failures tests

    - spec/sanity_spec.rb
     1. create fil with requirement of rails helper
     - require 'rails helper'

     2. describe sanity test with check test
     - describe :sanity do
     -   it 'works!' do
     -     expect(1).to eq 1
     -   end
     ...
     3. check that test works in console
     $ rspec
...
-------------------------------------------------------------------------------------------------------------------
    - client/assets/javascripts/component_render_server.js
     1. create file for component rendering with server and add requirements
     ...
     3. go to client/assets/javascripts folder and run supervisor
     - supervisor --harmony component_render_server.js

     4. checking: browser
     - localhost:3001
     - here you should get 'hello' message

     5. add add render arguments of post method
     ...
     7. add requirement of React
     - var React = require('react');

     8. add toUnderscore function
     - function toUnderscore(str) {
     -   return str.replace(/([A-Z])/g, function ($1) { return "_" + $1.toLowerCase(); }).slice(1);
     - }

    - client/assets/javascripts/stores/comment_store.js
     # add requirement of lodash
     - import _ from 'lodash'

    - Procfile.dev
     1. add supervisot to configurations
     ...
     3. checking: browser
     - localhost:3001
     - error that says 'Cannot GET/'

    - Gemfile
     1. add httparty gem
     - gem 'httparty', '~> 0.13.7'

     2. make active this gem
    ...
    - app/views/restaurants/show.html.erb
     # add isomorphic rendering
     - <%= isomprphic('CommentSection', 'cs', props) %>

    - app/helpers/react_helper
     # add isomorphic method with its functionality
    ...
    - client/assets/javascripts/component_render_server.js
     1. returning of 'res' using in app.post(...) method
     - app.post('/', function( req, res) {
     ...
     2. checking: browser
     - now in source of code you should get data with second comment array that contains different stuff
     - this stuff need the parsing, now this seems like this:  '... comments: [{&qout;id....}] ...'

    - app/views/restaurants/show.html.erb
     1. add JSON parsing for comments array
     # - <% props = JSON.parse("{\"restaurantId\": #{@restaurant.id},
     # -                         \"comments\": #{raw(render(template: 'comments/index.json.jbuilder'))}}") %>

    - app/helpers/react-helper.rb
     # add to_json method for props in react rendering
     - ...
     -   renderReact('#{id}', '#{component}', #{props.to_json});
     - ...

    - client/assets/javascripts/component_render_server.js
     1. add toString method for props of 'res'
     - ...
     -   return res.end(req.body.props.toString());
     - ...

     2. checking: browser
     - in page's source you should get [Object Object]

     3. delete return of res

     4. checking: browser
     - now you should get react's stuff on your page

    - app/helpers/react_helper.rb
     # add dif with applyaing of html_safe
     - def isomorphic(component, id, props)
    ...
    - app/views/restaurants/show.html.erb
     1. change cs for special types
     - <%= client('CommentSection', cs_client, props)%>
     - <%= isomorphic('CommentSection;, cs_server, props)%>

     2. checking: browser
     - check the source of page and you will see that your application has isomorphic rendering
     - this rendering seemed weirdo for reading

     3. delete client rendering to aviud double rendering of data to page
     - <%= isomorphic('CommentSection;, cs_server, props)%>

     4. checking: browser
     - now should be rendered only data from cs_server

     5. add condition for client and server
     ...
     6. checking: browser
     - view-source:http://localhost:4000/restaurants/1?client=1
     - should return only client type
     - view-source:localhost:4000/restaurants/1
     - should return isomorphic type

   - client/assets/javascripts/actions.js
     1. back watchInterval to constructor if it was under comment
     - constructor(props) {
     -   ...
     -   this.watchInterval = setInterval(this.watch.bind(this), 1000)
     - }

     2. check does everythink works in real time



-------------------------------------------------------------------------------------------------------------------
                    Building Processes With Foreman
-------------------------------------------------------------------------------------------------------------------
    - Gemfile
     1. add last foreman gem and install it
     - gem 'foreman', '~> 0.82.0'
...
-------------------------------------------------------------------------------------------------------------------
                    Bringing Rails and Webpack - III part (refactor with helper)
-------------------------------------------------------------------------------------------------------------------
    - app/views/restaurants/show.html.erb
     1. instead of script replace rendering with 'client' helper
     - <%= client('CommentSection',
                  'cs',
                  {restaurantId: @restaurant.id, comments: raw(render(template: 'comments/index.json.jbuilder'))})%>

     2. checking: browser
     - you should get error about undefined 'client' method

    - app/helpers/react_helper.rb
     # create helper with client and private script method that describe previous script's functionality
     - module ReactHelper
     -  def clinet(component, id, props)
     -    sc = script(component, id, props)
    ...
    - app/views/restaurants/show.html.erb
    # add refactoring for client's rendering and add interpolation for parameters
    <% props = "{restaurantId: #{@restaurant.id}, comments: #{raw(render(template: 'comments/index.json.jbuilder'))}}" %>
    <%= client('CommentSection, 'cs', props)%>

    - client/assets/javascripts/App.js
    1. add 'CommentSection' import in other way
    - var component = require("./components/" + toUnderscore(component));

    2. add toUnderscore function above
    - function toUnderscore(str) {
    -   return str.replace(/([A-Z])/g, function ($1) { return "_" + $1.toLowerCase(); }).slice(1);
    - }

    3. checking: browser
    - now rendering should works, just check it

   - client/assets/javascripts/components/comment_section.js
    # delete windowing for CommentSection


-------------------------------------------------------------------------------------------------------------------
                    Bringing Rails and Webpack - II part (page rendering)
-------------------------------------------------------------------------------------------------------------------
    - config/application.rb
     # delete es6 sprockets configuration stuff

    - Gemfile
     # delete not required stuff

    - app/views/restaurants/show.html.erb
     # add script configuration stuff
     - <script>
     -  document.addEventListener('DOMContentLoaded', function(ev) {
     -    renderReact('commentSection', CommentSection, {
    ...
    - app/client/assets/javascripts/components/comment_section.js
     1. delete JSON parsing of comments inside constructor for setComments method
     - cnostructor(props) {
     -   ...
     ...
     2. browser: checking
     - now components shoud render to page

-------------------------------------------------------------------------------------------------------------------
                    Bringing Rails and Webpack
-------------------------------------------------------------------------------------------------------------------
 I. Modules Adding
   1. app_dispatcher.js
    # import Dispatcher and delete 'Flux' keyword from new object creating
    - import { Dispatcher } from 'flux';
    - let AppDispatcher = new Dispatcher();

   2. components/comment_store.js
    # add import of EventEmitter from events
    - import { EventEmitter } from 'events';

   3. components/comment_seciton.js
    - import React from 'react';

   4. components/comment_list.js
    - import React from 'react';

   5. components/comment_form.js
    - import React from 'react';

   6. components/comment.js
    - import React from 'react';

   7. localhoset:3000
    - now u shouldn't get any error in console

 II. Changing to webpack format in view
  - app/views/restaurants/show.hrml.haml
   # delete react_component(...) and instead just add tag with id 'commentSection'
   - %ul.comments
   -   %li#commentSection

  - app/client/assets/javascripts/App.js
   1. render elements from App.js
   - import React from 'react';
   ...
   2. localhost:3000
   - open console and type in: 'window.renderReact'
   - u should get function with its parameters: function window.renderReact(id, component, props)
   - type in same stuff but at this time with parameters: 'window.renderReact('commentSection', CommentSection, {})
   - u should get error about undefined lodash's function
   - try to find source of problem for future fixing

  - app/client/assets/javascripts/api.js
   1. add import of lodash
   - import _ from 'lodash';

   2. localhoset:3000
   - now u should get error about problem in JSON

  - app/client/assets/javascripts/components/comment_section
   1. delete JSON parsing for 'setComments' function
   - constructor() {
   -   ...
   -   this.actions.setComments(props.comment);
   - }

   2. localhost:3000
   - enter: window.renderReact('commentSection', CommentSection, {restaurantId: 2, comments: []})
   - now you should get all your elements rendered
...
-------------------------------------------------------------------------------------------------------------------
   ...
   - app/assets/javascripts/
    $ mv api.js app_dispatcher.js constants.js ../../../client/assets/javascripts
    $ mv stores ../../../client/assets/javascripts
    $ mv components ../../../client/assets/javascripts
    $ mv actions.js ../../../client/assets/javascripts
    $ rm components.js

   - app/assets/javascripts/application.js
    1. delete already not required stuff
    - //= require jquery
    - //= requier jquer_ujs
    - //= require bootstrap-sprockets
    - //= require generated/client-bundle

    2. localhost:3000/restaurants/
    - use Ctrl + U to look at source of page
    - find script with '/assets/generated/client-bundle.slef-cb...' path and open its
    - this file should contain javascript stuff that we were dealing

   - client/assets/javascripts/App.js
    1. add alert of any stuff
    - aler('nice, this works')

    2. localhost:3000/restaurants/
    - you should get alert with added text

    3. back to file and instead of text alert add import of CommentSection
    - import CommentSection from './components/comment_section'

    4. check console with started webpack
    - you should get whole of errors that say webpack can't find certain module

   - client/assets/javascripts/components/
    # add changes to path of every component
   ...
   - client/assets/javascripts/stores/comment_store.js
    1. comment_store.js
    - import AppDispatcher form '../app_dispatcher';
    - import Constants from '../constants';

    2. localhost:3000/restaurants
    - check source of page
    - find client_bundle file
    - try to find 'CommentSection' or other component
    - also go to page of one restaurant, in console you should get error like: 'Flux... is not defined'
